...
void homePage()
{
    ...
    if (t12_error == 0 && t12_adc_error == 0) //正常画面
    {
        ...
        t12_temp >= 99 ? u8g2.setCursor(0, zjm_dh_y + 18) : u8g2.setCursor(10, zjm_dh_y + 18);

        if (t12_switch == 1 && (thisError_average <= 4 && thisError_average >= -4))
        {
            if (t12_temp_average - t12_temp_display > 0)
                t12_temp_display += 1;
            else if (t12_temp_average - t12_temp_display < 0)
                t12_temp_display -= 1;
        ...
        {
            if (t12_temp - t12_temp_display > 0)
                t12_temp_display += 1;
            else if (t12_temp - t12_temp_display < 0)
                t12_temp_display -= 1;
            u8g2.print(t12_temp_display);
        }
    }
    else if (t12_adc_error == 1)
        zjm_error2(); //T12ADC超出测量范围画面
    else if (t12_error == 1)
        zjm_error3(); //手柄未接显示画面
    //****** 画设置温度进度条
    ...
    if (set_temp <= 310)
        u8g2.setCursor(zjm_x1 + 5, zjm_dh_y + 29); //画设置温度
    else
        u8g2.setCursor(zjm_x1 - 21, zjm_dh_y + 29); //画设置温度
    u8g2.print(set_temp);
    ...
    if (/*millis() - zjm_dh_time1 > 30 &&*/ zjm_dh_run == 1)
    {
        if (zjm_dh_x1 < 7)
        {
            ...
            {
                zjm_dh_r1 = 0;
                if (t12_switch == 1 && zjm_sleep_ts == 0)
                    EEPROM.put(set_temp_eeprom, set_temp); //停止时保存当前的设置温度
    ...
    if (t12_switch == 1 && zjm_sleep_ts == 0)
        u8g2.print("热");
    else if (t12_switch == 1 && zjm_sleep_ts == 1)
        u8g2.print("休");
    else
        u8g2.print("停");
    if (zjm_dh_x1 == 7)
    ...
    if (t12_switch == 1)
    {
        ...
        u8g2.drawHLine(74, zjm_dh_y + 31, 14);

    //****** 画室温-电压-电流-工作状态
    if (vin_error == 1)
        zjm_error1(); //电压电流有一个报警就显示 报警提示画面
    else
        zjm_vin_ec_temp_state(); //显示正常画面
